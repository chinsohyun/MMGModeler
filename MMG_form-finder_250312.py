...
class RemoteControlPanel(forms.Form):
    ...
    def load_trajectory(self, sender, e):
        try:
            ...
            self.seeds += seeds
            self.coords += data["geometry"]["coords"]
            self.lines += lines
    ...
    def on_redo_button_click(self, sender, e):
        ...
        try:
            ...
            self.action_log()
            self.redo = False
            #print("get length!", len(self.coords), len(self.lines), len(self.shapes), len(self.simples), len(self.seeds))
