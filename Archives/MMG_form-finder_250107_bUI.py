    ...
    def on_run_button_click(self, sender, e):
        try:
            ...
            if mode == 0: #Element
                ...
                element_mode(center_cord, 0, 0, threshold, container, [], [])
...
def element_mode(pt_cord, rotation, mode, threshold, container, three_set, center_mark):
...
#    width = 1 #need further modification
    ...
    if rotation %3 == 0: #need further modification
        ...
        for start_cord in container:
            translation = (start_cord[0]-0.5, start_cord[1]-0.5, start_cord[2])
            rect_center = rs.AddRectangle(xy, 1, 1)
            rs.MoveObject(rect_center, translation)
            start_points.append(rs.AddPoint(start_cord))
            temp_circle.append(rect_center)
            
        #pt_cord updated
        pt_cord = rs.GetPoint("Select a new center: ") #, container[-1]
        
        if rotation == 0 and pt_cord != (0,0,0):
            container[0] = pt_cord
        ...
        center_a = rs.AddRectangle(xy, 1.5, 1.5)
        ...
        ###recheck, next_cord not in container
        print("initial", next_cord)
    ...
    if rotation % 3 == 1:  ###############
        ...
        next_cord = rs.GetPoint("Select the next point: ")#, next_points[0]
        print(next_cord, "was it filtered")
...
#                if item != tuple(container[-1]):
        ...
        print("second", next_cord)
        ps = draw_mode(thk, dim, pt_cord, next_cord)
        three_set=[]
        
    container.append(next_cord)
    element_mode(pt_cord, rotation + 1, mode, threshold, container, three_set, center_mark)
